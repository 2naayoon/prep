...
-- SALARY 가 2500, 3500, 7000 이 아니며 JOB_ID 이 SA_REP 나 ST_CLERK 사원 조회
SELECT *
FROM EMPLOYEES e 
...
-- 매니저가 관리하는 사람들 중에서 최소급여가 6000 미만인 최소급여는 제외
-- 메니저가 없는 사원 제외
-- 102 9000
-- 103 8300
-- 101 5800 [X]
...
FROM EMPLOYEES e 
WHERE SALARY > (SELECT MAX(SALARY) FROM EMPLOYEES e WHERE JOB_ID = 'SA_MAN');

-- 커미션을 버는 사원들의 부서와 연봉이 동일한 사원들의 LAST_NAME, 부서번호, SALARY 조회
...
WHERE DEPARTMENT_ID IN (SELECT DISTINCT DEPARTMENT_ID 
						FROM EMPLOYEES e
						WHERE SALARY * 12 > (SELECT AVG(SALARY * 12) FROM EMPLOYEES e)
							AND LAST_NAME LIKE '%u%');
	
-- 각 부서별 평균 연봉보다 더 받는 동일 부서 근무사원들의 LAST_NAME, 부서번호, 연봉 및 해당 부서의 평균 연봉 조회
-- 부서별 연봉을 기준으로 정렬
SELECT E1.LAST_NAME, E1.DEPARTMENT_ID, E1.SALARY * 12 AS 연봉, ROUND(E2.DEPT_SAL_AVG) AS DEPT_SAL_AVG
FROM EMPLOYEES E1,
	 (SELECT DEPARTMENT_ID, AVG(SALARY * 12) AS DEPT_SAL_AVG FROM EMPLOYEES e GROUP BY DEPARTMENT_ID) E2
WHERE E1.DEPARTMENT_ID = E2.DEPARTMENT_ID AND E1.SALARY * 12 > E2.DEPT_SAL_AVG
ORDER BY E2.DEPT_SAL_AVG;

-- LAST_NAME이 Davies 인 사람보다 후에 고용된 사원들의 LAST_NAME, HIRE_DATE 조회
SELECT LAST_NAME, HIRE_DATE 
FROM EMPLOYEES e 
WHERE HIRE_DATE > (SELECT HIRE_DATE FROM EMPLOYEES e WHERE LAST_NAME = 'Davies');

-- LAST_NAME이 King 인 사원을 매니저로 두고 있는 모든 사원들의 LAST_NAME, SALARY 조회
SELECT LAST_NAME, SALARY
FROM EMPLOYEES e 
WHERE MANAGER_ID IN (SELECT EMPLOYEE_ID FROM EMPLOYEES e WHERE LAST_NAME = 'King');

-- LAST_NAME이 Kochhar 인 사원과 동일한 연봉 및 커미션을 버는 사원들의 LAST_NAME, 부서번호, SALARY 조회
-- Kochhar 은 제외한다
SELECT LAST_NAME, DEPARTMENT_ID, SALARY
FROM EMPLOYEES e 
WHERE (SALARY * 12, NANVL(COMMISSION_PCT, 0)) IN (
	SELECT SALARY * 12, NANVL(COMMISSION_PCT, 0) FROM EMPLOYEES e WHERE LAST_NAME = 'Kochhar')
...
WHERE c.COUNTRY_ID IN (
	SELECT l.country_ID 
	FROM LOCATIONS l, (SELECT LOCATION_ID FROM DEPARTMENTS) D
...
SELECT DISTINCT (SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES e) AS "총 사원수", 
	(SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES e WHERE TO_CHAR(HIRE_DATE, 'YY') = '13') AS "2013",
	(SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES e WHERE TO_CHAR(HIRE_DATE, 'YY') = '14') AS "2014",
	(SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES e WHERE TO_CHAR(HIRE_DATE, 'YY') = '15') AS "2015",
	(SELECT COUNT(EMPLOYEE_ID) FROM EMPLOYEES e WHERE TO_CHAR(HIRE_DATE, 'YY') = '16') AS "2016"
FROM EMPLOYEES e;
