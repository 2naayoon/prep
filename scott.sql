...
-- 열이름(컬럼명) : USERNAME → USER_NAME (이름에 "_" 사용 = Snakecase)
CREATE TABLE TBL_NOTNULL (
	LOGIN_ID VARCHAR2(20) NOT NULL,
	LOGIN_PWD VARCHAR2(20) NOT NULL,
	TEL VARCHAR2(20));

-- *Error : NULL을 ("SCOTT"."TBL_NOTNULL"."LOGIN_PWD") 안에 삽입할 수 없습니다
...
-- *Error : 무결성 제약 조건(SCOTT.SYS_C008362)에 위배됩니다
INSERT INTO TBL_UNIQUE
VALUES ('TESTID1', 'TESTID1', '010-1234-5678'); - 중복
...
-- FOREIGN KEY (외래키) : 특정 테이블의 기본키로 지정한 열을 다른 테이블의 특정 열에서 참조
-- 부모키 / 자식키
...
-- *Error : 무결성 제약조건(SCOTT.EMPFK_DEPTNO_FK)이 위배되었습니다- 부모 키가 없습니다
-- → 참조 대상 테이블에 데이터 INSERT
INSERT INTO EMP_FK VALUES (1000, 'HONG', 'CLERK', 7788, SYSDATE, 1200, NULL, 50);

INSERT INTO DEPT_FK VALUES (50, 'DATABASE', 'SEOUL');

-- *ERROR : 무결성 제약조건(SCOTT.EMPFK_DEPTNO_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
-- → 참조하는 테이블의 자식 먼저 DELETE
...
-- *Error : 체크 제약조건(SCOTT.TBLCK_PWD_CK)이 위배되었습니다
...
-- *Error : 체크 제약조건(SCOTT.TBLCK2_GRADE_CK)이 위배되었습니다
...
INSERT INTO TBL_DEFAULT VALUES ('TESTID1', '5678', NULL);
...
VALUES ('hong123', 'hong123', '김지호', 15, '학생');
...
VALUES (BOARD_SEQ.NEXTVAL, '게시글1', '내용');
